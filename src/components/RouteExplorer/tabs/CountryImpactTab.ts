...
export class CountryImpactTab {
  ...
  private renderMissing(): void {
    this.element.innerHTML =
      ...
      '<p>ArcaneWatchtower does not have bilateral trade data for this destination country yet.</p>' +
      '</div>';
  }
  ...
  private renderLazy(): void {
    this.element.innerHTML =
      ...
      '<p>ArcaneWatchtower is fetching trade data for this destination for the first time. ' +
  ...
  private renderData(data: GetRouteImpactResponse): void {
    const bannerHtml = !data.hs2InSeededUniverse
      ? '<div class="re-impact__banner">Lane value for this HS code is not in ArcaneWatchtower\'s strategic-products dataset. Top strategic products shown below.</div>'
      : '';
