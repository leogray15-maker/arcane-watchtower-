...
export function exportToJSON(data: ExportData, filename = 'arcane-export'): void {
  const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'arcane-export'): void {
  const clean = sanitizeData(data);
  const lines: string[] = [];

  lines.push(`# ArcaneWatchtower Export — ${new Date(clean.timestamp).toISOString()}`);
