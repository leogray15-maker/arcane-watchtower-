...
const DEFAULT_WEB_API_URL = 'https://api.arcane-watchtower.vercel.app';
...
function isArcaneWatchtowerWebHost(hostname: string): boolean {
  return hostname === 'arcane-watchtower.vercel.app'
    || hostname === 'www.arcane-watchtower.vercel.app'
    || hostname.endsWith('.arcane-watchtower.vercel.app');
}
...
export function getConfiguredWebApiBaseUrl(): string {
  ...
  if (!isArcaneWatchtowerWebHost(hostname)) {
    return '';
  }
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://arcane-watchtower.vercel.app';
  return '';
}
...
const APP_HOSTS = new Set([
  'arcane-watchtower.vercel.app',
  'www.arcane-watchtower.vercel.app',
  'tech.arcane-watchtower.vercel.app',
  'api.arcane-watchtower.vercel.app',
  'localhost',
  '127.0.0.1',
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.arcane-watchtower.vercel.app');
  } catch {
    return false;
  }
}
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        if (wmKeyValue) {
          cloudHeaders.set('X-ArcaneWatchtower-Key', wmKeyValue);
        }
      }
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDMONITOR_API_KEY from runtime config → X-ArcaneWatchtower-Key
   *   3. Tester key (wm-pro-key / wm-widget-key) → X-ArcaneWatchtower-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (testerKey) {
      headers.set('X-ArcaneWatchtower-Key', testerKey);
      return { ...init, headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.arcane-watchtower.vercel.app/api/...) — just inject auth.
