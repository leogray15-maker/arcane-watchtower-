...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://arcane-watchtower.vercel.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-ArcaneWatchtower-Key')) {
  ...
  try {
    ...
    if (wmKey) {
      existing.set('X-ArcaneWatchtower-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-ArcaneWatchtower-Key', testerKey);
