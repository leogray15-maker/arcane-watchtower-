    ...
    attach(container: HTMLElement): () => void {
          ...
          upgradeBtn.addEventListener('click', () => {
            if (!host.isSignedIn) {
              import('@/services/clerk').then(m => m.openSignIn()).catch(() => {
                window.open('https://arcane-watchtower.vercel.app/pro', '_blank');
              });
              return;
            }
      ...
      if (!host.isDesktopApp && host.isSignedIn && hasTier(1)) {
        ...
        container.addEventListener('click', (e) => {
          ...
          const startTelegramPairing = (rowEl: HTMLElement) => {
            ...
            createPairingToken().then(({ token, expiresAt }) => {
              if (signal.aborted) return;
              const botUsername = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_BOT_USERNAME as string | undefined) ?? 'ArcaneWatchtowerBot';
        ...
        const onMessage = (e: MessageEvent): void => {
          // Bind trust to both: (1) a WM-owned origin (callback is always on arcane-watchtower.vercel.app,
          // but settings may be open on a different *.arcane-watchtower.vercel.app subdomain) and
          ...
          const trustedOrigin = e.origin === window.location.origin ||
            e.origin === 'https://arcane-watchtower.vercel.app' ||
            e.origin === 'https://www.arcane-watchtower.vercel.app' ||
            e.origin.endsWith('.arcane-watchtower.vercel.app');
