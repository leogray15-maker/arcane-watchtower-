...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      const res = await fetch('https://api.arcane-watchtower.vercel.app/api/version', {
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `https://api.arcane-watchtower.vercel.app/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {
