...
Sentry.init({
  ...
  environment: (location.hostname === 'arcane-watchtower.vercel.app' || location.hostname.endsWith('.arcane-watchtower.vercel.app')) ? 'production'
...
// In web production, route RPC calls through api.arcane-watchtower.vercel.app (Cloudflare edge).
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('arcane-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('arcane-beta-mode', 'true');
    else localStorage.removeItem('arcane-beta-mode');
    location.reload();
  },
});
